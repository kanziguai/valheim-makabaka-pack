...
**当前版本：v1.13（2026-09-19）** · 安装包 82.8 MB（不含模型；模型按需下载，详见 `模型说明.txt`）· 见 [Releases](../../releases)
...
- 想先看模型长什么样：**选模型时直接输 `p` 打开总览图**（一张图看全部 15 个、图上编号 = 菜单序号），`p3` 看第 3 个的大图，`f` 打开参考图文件夹；图也在包内 `Models\预览\`（450×700 JPEG，约 0.45 MB）。
